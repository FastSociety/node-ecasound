    ...
    ECASound.prototype.play = function(fComplete) {
        ...
        this.launch(aCommand, fComplete);
    };

    ECASound.prototype._normalizeTracks = function(aTracks, fCallbackAsync) {
        if (aTracks.length) {
            var sTimer = syslog.timeStart('ECASound._normalizeTracks', {command: 'ecanormalize ' + aTracks});
            console.log('ecanormalize ' + aTracks);
            var ecanormalize = spawn('ecanormalize', aTracks.join(' ').split(' '));
            var aOut   = [];
            var aError = [];
            var iCode  = 0;

            ecanormalize.stderr.on('data', function (err)    {
                aError.push(err.toString());
            });

            ecanormalize.stdout.on('data', function (output) {
                console.log(output);
                aOut.push(output.toString());
            });

            ecanormalize.on('exit',        function (code)   {
                iCode = code;
            });

            ecanormalize.on('error',        function (err)   {
                aError.push(err.toString());
            });

            ecanormalize.on('close',        function (code)   {
                    syslog.timeStop(sTimer);
                    if (iCode > 0) {
                        if (aError.length) {
                            fCallbackAsync(aError);
                        } else {
                            fCallbackAsync('Code ' + iCode);
                        }
                    } else {
                        if (aError.length) {
    ...
    ECASound.prototype.saveAs = function(sFileName, fComplete) {
        ...
        for (var i in this.aTracks) {
            var oTrack = this.aTracks[i];
            aCommand.push(oTrack.command());
            if (oTrack.mNormalize)
                aNormalizeFiles.push(oTrack.sFile);
            else
                console.log("Not Normalizing ", oTrack.sFile);
        }
        ...
        this._normalizeTracks(aNormalizeFiles, function(err){
            if (err)
                console.log("Error normalizing track (Non-Fatal, continuing): ", err);
            this.launch(aCommand, sFileName, fComplete);    
        }.bind(this));
    };
     ...
     */
    ECASound.prototype.launch = function(aCommand, sFileName, fComplete) {
        var sTimer = syslog.timeStart('ECASound.spawn', {command: 'ecasound' + aCommand.join(' ').split(' ')});
    ...
    ECASound.Track.prototype.normalize = function(bNorm) {
        this.mNormalize = bNorm;
