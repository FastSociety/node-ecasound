    ...
    ECASound.prototype._normalizeTracks = function(aTracks, fCallbackAsync) {
        if (aTracks.length) {
            var sTimer = syslog.timeStart('ECASound._normalizeTracks', {command: 'ecanormalize ' + aTracks.join(' ')});
            console.log('exec(ecanormalize ' + aTracks.join(' ') + ')');

            exec('ecanormalize ' + aTracks.join(' '), function(oError, oStdOut, oStdErr) {
                    syslog.timeStop(sTimer);
                    
                    if (oError !== null) {
                        syslog.error({ action: sTimer, stdErr: oError, stdOut: oStdOut, stdErr: oStdErr });
                    }
                    else     
                        syslog.debug({ action: sTimer, stdErr: oError, stdOut: oStdOut, stdErr: oStdErr });
    ...
    ECASound.prototype.saveAs = function(sFileName, fComplete) {
        ...
        this._normalizeTracks(aNormalizeFiles, function(err){
            if (err)
                syslog.warn({ action: 'ECASound.prototype.saveAs', msg: 'Error normalizing track (Non-Fatal, continuing)'});
