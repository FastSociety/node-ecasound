    ...
    ECASound.prototype._normalizeTracks = function(aTracks, fCallbackAsync) {
        if (aTracks.length) {
            var sTimer = syslog.timeStart('ECASound._normalizeTracks', {command: 'ecanormalize ' + aTracks.split(' ')});
            console.log('exec(ecanormalize ' + aTracks.split(' '));

            exec('ecanormalize ' + aTracks.split(' '), function(oError, oStdOut, oStdErr) {
                    syslog.timeStop(sTimer);
                    
                    if (oError !== null)
                        console.log('ecanormalize oError: ' + oError);

                    console.log('ecanormalize stdOut: ' + oStdOut);
                    console.log('ecanormalize stdErr: ' + oStdErr);
